<div class="mx-auto flex w-full max-w-[430px] h-fit self-center justify-self-center flex-col gap-[12px] rounded-[24px] bg-white p-6 sm:p-[clamp(24px,2vw,32px)] shadow-xl border border-[#E5E7EB]">
    <div class="space-y-3 text-center">
        ...
        <div>
            <h2 class="text-[28px] sm:text-[36px] font-semibold tracking-[-0.03em]"><span class="text-[#2563EB]">Selamat</span> Datang</h2>
    ...
    <div class="space-y-[12px]">
        ...
        <div class="space-y-2" x-data="{ show: false }">
            <label for="password" class="block text-[14px] font-medium text-slate-700">Kata Sandi</label>
            <div class="relative flex h-[46px] items-center rounded-xl border border-slate-300 bg-white shadow-sm transition duration-200 focus-within:border-[#2563EB] focus-within:ring-0">
                ...
                <input :type="show ? 'text' : 'password'" id="password" name="password" required autocomplete="current-password" placeholder="Masukkan Kata Sandi" class="h-full w-full border-0 bg-transparent pl-[44px] pr-[44px] text-sm text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-0" />
        ...
        <div class="flex items-center justify-between text-[14px] text-slate-600">
            ...
            @if(Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-[14px] font-medium text-[#2563EB] hover:text-[#1D4ED8]">Lupa Kata Sandi?</a>
            @endif
        </div>
        ...
        <div class="flex items-center gap-3 text-[14px] text-slate-400 py-2">
            <span class="h-px flex-1 bg-[#E5E7EB]"></span>
            <span>atau</span>
            <span class="h-px flex-1 bg-[#E5E7EB]"></span>
        </div>

        <a href="{{ route('devices.create') }}" class="inline-flex h-[46px] w-full items-center justify-center gap-2 rounded-xl border border-[#2563EB] bg-white text-[14px] font-semibold text-[#2563EB] transition duration-200 hover:bg-[#EFF6FF]">
